...
if __name__ == '__main__':
    ...
    cat_names = [x['name'] for x in sorted(data['categories'], key=lambda x:
        x['id'])]
    if 'synonyms' in data['categories'][0]:
        if args.use_wn_name:
            synonyms = [([xx.name() for xx in wordnet.synset(x['synset']).
                lemmas()] if x['synset'] != 'stop_sign.n.01' else [
                'stop_sign']) for x in sorted(data['categories'], key=lambda
                x: x['id'])]
        else:
            synonyms = [x['synonyms'] for x in sorted(data['categories'],
                key=lambda x: x['id'])]
    else:
        synonyms = []
    if args.fix_space:
    ...
    if args.use_underscore:
        cat_names = [x.strip().replace('/ ', '/').replace(' ', '_') for x in
            cat_names]
...
>>>    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    ...
    if args.prompt == 'none':
        ...
        sentences_synonyms = [['a photo of a {}'.format(xx) for xx in x] for
            x in synonyms]
    elif args.prompt == 'scene':
        sentences = ['a photo of a {} in the scene'.format(x) for x in
            cat_names]
        sentences_synonyms = [['a photo of a {} in the scene'.format(xx) for
            xx in x] for x in synonyms]
    print('sentences_synonyms', len(sentences_synonyms), sum(len(x) for x in
        sentences_synonyms))
    if args.model == 'clip':
        import clip
        print('Loading CLIP')
        ...
        with paddle.no_grad():
            if len(text) > 10000:
                text_features = paddle.concat(x=[model.encode_text(text[:
                    len(text) // 2]), model.encode_text(text[len(text) // 2
                    :])], axis=0)
            else:
...
>>>            text_features = text_features.split(synonyms_per_cat, dim=0)
            ...
            text_features = paddle.stack(x=text_features, axis=0)
