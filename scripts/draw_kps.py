...
pd_kps_file = (
    'checkpoints/DetrKeypoint/0503_e2e4box_min1/results_keypoints.json')
img_dir = '/root/coco17/val2017'
save_dir = 'checkpoints/DetrKeypoint/0503_e2e4box_min1/results_kps'
THR = 0.2
CAT_IDS = None
skeleton = [[16, 14], [14, 12], [17, 15], [15, 13], [12, 13], [6, 12], [7, 
    13], [6, 7], [6, 8], [7, 9], [8, 10], [9, 11], [2, 3], [1, 2], [1, 3],
    [2, 4], [3, 5], [4, 6], [5, 7]]
...
def drawImg(img_id, gt_ann, pd_kps, coco_eval):
    ...
    gt_keypoints = np.asarray([ann['keypoints'] for ann in gt_anns]).reshape((
        -1, 17, 3))
    ...
    pd_keypoints = np.asarray([ann['keypoints'] for ann in pd_anns]).reshape((
        -1, 17, 3))
    ...
    if len_pd == 0:
        return
    iou_table = coco_eval.ious[img_id, 1][:len_pd]
    cls_matched_gt, cls_matched_pd = matchKeypoints(gt_keypoints.reshape((-
        1, 17, 3)), pd_keypoints.reshape((-1, 17, 3)), iou_table)
    ...
    for cls_name, box, keypoint, matched in zip(gt_cls, gt_box,
        gt_keypoints, gt_matched):
        ...
        cv2.putText(image, cls_name, (box[0], box[3]), font, font_scale, (0,
            0, 0), lineType=cv2.LINE_AA)
    for cls_name, keypoint, matched, score in zip(pd_cls, pd_keypoints,
        pd_matched, pd_score):
        ...
        txt_st, txt_ed = (keypoint[0, 0], keypoint[0, 1] - txt_h), (
            keypoint[0, 0] + txt_w, keypoint[0, 1])
        cv2.rectangle(image, txt_st, txt_ed, color, -1)
        for isk in skeleton:
        ...
        for ipt in keypoint:
            cv2.circle(image, (ipt[0], ipt[1]), 3, color)
        cv2.putText(image, text, (keypoint[0, 0], keypoint[0, 1]), font,
            font_scale, (0, 0, 0), lineType=cv2.LINE_AA)
