...
_C.DATA.COCO_HYBRID.detection_anno = (
    'data/coco/annotations/instances_{}2017.json')
_C.DATA.COCO_HYBRID.keypoint_anno = (
    'data/coco/annotations/person_keypoints_{}2017.json')
_C.DATA.IMNET = CN()
...
BASIC_LAYER_CFG.dropout = 0.0
BASIC_LAYER_CFG.self_attn_dropout = 0.0
...
_C.MODEL.PROMPT_INDICATOR.level_preserve = []
...
_C.MODEL.PROMPT_INDICATOR.CLASS_PROMPTS.init_vectors = ''
...
_C.MODEL.OBJECT_DECODER.HEAD.task_category = (
    'configs/tasks/coco_detection.json')
...
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.class_normalization = 'num_box'
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.keypoint_criterion = 'L1'
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.keypoint_normalization = 'num_box'
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.oks_normalization = 'num_box'
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.keypoint_reference = 'absolute'
...
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.MATCHER.set_class_type = 'focal'
...
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.MATCHER.set_box_normalization = 'none'
...
_C.MODEL.OBJECT_DECODER.HEAD.LOSS.MATCHER.set_keypoint_reference = 'absolute'
_C.TRAIN = CN()
_C.TRAIN.epochs = 50
...
def _update_config_from_file(config, cfg_file):
    ...
    for cfg in yaml_cfg.setdefault('BASE', ['']):
        if cfg:
            _update_config_from_file(config, os.path.join(os.path.dirname(
                cfg_file), cfg))
...
def post_process(config):
    ...
    if 'Seq' in config.MODEL.OBJECT_DECODER.HEAD.type:
        ...
        config.MODEL.OBJECT_DECODER.HEAD.update(config.MODEL.OBJECT_DECODER
            .LAYER)
        config.MODEL.OBJECT_DECODER.HEAD.cross_attn_no_value_proj = old_no_proj
        config.MODEL.OBJECT_DECODER.HEAD.LOSS.task_category = (config.MODEL
            .OBJECT_DECODER.HEAD.task_category)
        config.MODEL.OBJECT_DECODER.HEAD.LOSS.num_classes = (config.MODEL.
            OBJECT_DECODER.HEAD.num_classes)
