...
def get_args_parser():
    ...
    parser.add_argument('--opts', help=
        "Modify config options by adding 'KEY VALUE' pairs. ", default=None,
        nargs='+')
    return parser
...
def build_optimizer(cfg_train, model_without_ddp, steps_per_epoch=None):
    ...
    for group_keys, multiplier in zip(cfg_train.lr_groups, cfg_train.lr_mults):
        print('Params containing', group_keys, 'has lr', cfg_train.lr *
            multiplier)
        param_dicts.append({'params': [p for n, p in model_without_ddp.
            named_parameters() if match_name_keywords(n, group_keys) and p.
            requires_grad and not no_weight_decay_func(n, p)], 'lr': 
            cfg_train.lr * multiplier})
        param_dicts.append({'params': [p for n, p in model_without_ddp.
            named_parameters() if match_name_keywords(n, group_keys) and p.
            requires_grad and no_weight_decay_func(n, p)], 'lr': cfg_train.
            lr * multiplier, 'weight_decay': 0.0})
        except_keys += group_keys
    param_dicts.append({'params': [p for n, p in model_without_ddp.
        named_parameters() if not match_name_keywords(n, except_keys) and p
        .requires_grad and no_weight_decay_func(n, p)], 'lr': cfg_train.lr,
        'weight_decay': 0.0})
    param_dicts.append({'params': [p for n, p in model_without_ddp.
        named_parameters() if not match_name_keywords(n, except_keys) and p
        .requires_grad and not no_weight_decay_func(n, p)], 'lr': cfg_train.lr}
        )
    while len(param_dicts[0]['params']) == 0:
        param_dicts = param_dicts[1:]
    print('Param Distribution', [len(igroup['params']) for igroup in
        param_dicts])
    if cfg_train.sgd:
>>>        optimizer = torch.optim.SGD(param_dicts, lr=cfg_train.lr, momentum=
            0.9, weight_decay=cfg_train.weight_decay)
    else:
>>>        optimizer = torch.optim.AdamW(param_dicts, lr=cfg_train.lr,
            weight_decay=cfg_train.weight_decay)
    if cfg_train.sched == 'OneCycle':
>>>        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer,
            max_lr=cfg_train.lr, steps_per_epoch=steps_per_epoch, epochs=
            cfg_train.epochs, pct_start=0.2)
    elif cfg_train.sched == 'Step':
>>>        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, cfg_train
            .lr_drop)
    else:
        raise KeyError
...
>>>    model.to(device)
    ...
    n_parameters = sum(p.size() for p in model.parameters() if p.requires_grad)
    print('number of params:', n_parameters)
    dataset_train = build_dataset(image_set='train', args=args)
    dataset_val = build_dataset(image_set='val', args=args)
    data_loader_train, data_loader_val, sampler_train = build_dataloader(
        dataset_train, dataset_val, args.DATA)
    evaluate = getEvaluator(args)
    optimizer, lr_scheduler = build_optimizer(args.TRAIN, model_without_ddp,
...
>>>        model = torch.nn.parallel.DistributedDataParallel(model, device_ids
    ...
    if args.DATA.type == 'coco_panoptic':
        ...
        base_ds = get_coco_api_from_dataset(dataset_val
            ), dataset_val.prepare.contiguous_category_id_to_json_id
    else:
        base_ds = None
    if args.frozen_weights is not None:
...
>>>            checkpoint = torch.load(args.resume, map_location='cpu')
        ...
        if args.MODEL.PROMPT_INDICATOR.CLASS_PROMPTS.fix_class_prompts:
            ...
            del checkpoint['model'][
                'transformer.prompt_indicator.class_prompts']
        missing_keys, unexpected_keys = model_without_ddp.set_state_dict(
            state_dict=checkpoint['model'], strict=False)
        unexpected_keys = [k for k in unexpected_keys if not (k.endswith(
            'total_params') or k.endswith('total_ops'))]
        if len(missing_keys) > 0:
            print('Missing Keys: {}'.format(missing_keys))
        if len(unexpected_keys) > 0:
            print('Unexpected Keys: {}'.format(unexpected_keys))
        if (not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in
            checkpoint and 'epoch' in checkpoint):
            p_groups = copy.deepcopy(optimizer.param_groups)
            optimizer.set_state_dict(state_dict=checkpoint['optimizer'])
            for pg, pg_old in zip(optimizer.param_groups, p_groups):
                pg['lr'] = pg_old['lr']
                pg['initial_lr'] = pg_old['initial_lr']
            lr_scheduler.set_state_dict(state_dict=checkpoint['lr_scheduler'])
            ...
            if args.override_resumed_lr_drop:
                ...
                lr_scheduler.base_lrs = list(map(lambda group: group[
    ...
    if args.eval:
        ...
        if args.DATA.type == 'coco' and args.output_dir:
            for iou_type in coco_evaluator.iou_types:
                utils.save_on_master(coco_evaluator.coco_eval[iou_type].
                    eval, output_dir / f'eval_{iou_type}.pth')
            if utils.is_main_process() and args.EVAL.save_json:
                for iou_type in coco_evaluator.iou_types:
                    with open(os.path.join(args.output_dir,
                        f'results_{iou_type}.json'), 'w') as f:
    ...
    for epoch in range(args.start_epoch, training_epochs):
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.TRAIN.lr_drop == 0 or (epoch + 1
                ) % 10 == 0 or (epoch + 1
                ) % args.eval_interval == 0 or epoch >= training_epochs - 5:
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({'model': model_without_ddp.state_dict
                    (), 'optimizer': optimizer.state_dict(), 'lr_scheduler':
                    lr_scheduler.state_dict(), 'epoch': epoch, 'args': args
                    }, checkpoint_path)
        if (epoch + 1
            ) % args.eval_interval == 0 or epoch >= training_epochs - 5:
            test_stats, coco_evaluator = evaluate(model, postprocessors,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'AnchorDETR training and evaluation script', parents=[
        get_args_parser()])
