...
class CocoEvaluator(object):
    ...
    def synchronize_between_processes(self):
        for iou_type in self.iou_types:
            self.eval_imgs[iou_type] = np.concatenate(self.eval_imgs[
                iou_type], 2)
    ...
    def prepare_for_coco_detection(self, predictions):
        ...
        for original_id, prediction in predictions.items():
            ...
            coco_results.extend([{'image_id': original_id, 'category_id':
                self.contiguous_category_id_to_json_id[labels[k]], 'bbox':
                box, 'score': scores[k]} for k, box in enumerate(boxes)])
        return coco_results
    ...
    def prepare_for_coco_segmentation(self, predictions):
        ...
        for original_id, prediction in predictions.items():
            ...
            scores = prediction['scores'].tolist()
            labels = prediction['labels'].tolist()
            rles = [mask_util.encode(np.array(mask[(0), :, :, (np.newaxis)],
                dtype=np.uint8, order='F'))[0] for mask in masks]
            for rle in rles:
                rle['counts'] = rle['counts'].decode('utf-8')
            coco_results.extend([{'image_id': original_id, 'category_id':
                self.contiguous_category_id_to_json_id[labels[k]],
                'segmentation': rle, 'score': scores[k]} for k, rle in
                enumerate(rles)])
        return coco_results
    ...
    def prepare_for_coco_keypoint(self, predictions):
        ...
        for original_id, prediction in predictions.items():
            ...
            coco_results.extend([{'image_id': original_id, 'category_id':
                self.contiguous_category_id_to_json_id[labels[k]],
                'keypoints': keypoint, 'score': scores[k]} for k, keypoint in
                enumerate(keypoints)])
        return coco_results


def convert_to_xywh(boxes):
    xmin, ymin, xmax, ymax = boxes.unbind(axis=1)
    return paddle.stack(x=(xmin, ymin, xmax - xmin, ymax - ymin), axis=1)
