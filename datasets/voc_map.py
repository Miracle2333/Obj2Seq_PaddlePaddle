...
>>>    seg = np.array([x.strip().split(' ') for x in lines]).astype(float)
    ...
    results = {}
    pred_onehot = np.zeros_like(gt_label)
    for k in ks:
        indexs = sorted_inds[:, :k]
        np.put_along_axis(pred_onehot, indices=sorted_inds[:, :k], values=1,
            axis=1)
...
>>>    seg = np.array([x.strip().split(' ') for x in lines]).astype(float)
    ...
    for k in ks:
        ...
        recall = intersection.sum() / gt_label.sum()
        results[k] = recall
        counts[k] = pred_onehot.sum(axis=1).mean(), intersection.sum(axis=1
            ).mean(), gt_label.sum(axis=1).mean()
    save_dir = os.path.split(imagessetfilelist[0])[0]
>>>    torch.save({'pred_probs': pred_logits, 'gt_label': gt_label, 'recall':
        results, 'counts': counts}, os.path.join(save_dir, 'thr_recall.pth'))
