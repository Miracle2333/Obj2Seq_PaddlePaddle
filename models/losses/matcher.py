...
class HungarianMatcher(paddle.nn.Layer):
    ...
    def __init__(self, cost_class: float=1, cost_bbox: float=1, cost_giou:
        float=1):
        """Creates the matcher
        Params:
    ...
    def forward(self, outputs, targets):
        ...
        with paddle.no_grad():
            ...
            out_prob = outputs['pred_logits'].flatten(start_axis=0, stop_axis=1
                ).sigmoid()
            out_bbox = outputs['pred_boxes'].flatten(start_axis=0, stop_axis=1)
            tgt_ids = paddle.concat(x=[v['labels'] for v in targets])
...
def build_matcher(args):
    return HungarianMatcher(cost_class=args.set_cost_class, cost_bbox=args.
        set_cost_bbox, cost_giou=args.set_cost_giou)
