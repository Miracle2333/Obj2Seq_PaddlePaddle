...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            paddle.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise relative position index for each token inside the window
        coords_h = torch.arange(self.window_size[0])
        coords_w = torch.arange(self.window_size[1])
        coords = paddle.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww
...
class BasicLayer(nn.Layer):
    ...
    def __init__(self,
        ...
        if downsample is not None:
            self.downsample = downsample(axis=dim, norm_layer=norm_layer)
        else:
    ...
    def forward(self, x, H, W):
        ...
        img_mask = paddle.zeros((1, Hp, Wp, 1), device=x.device)  # 1 Hp Wp 1
...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_axis=96, norm_layer=None):
        super().__init__()
...
class SwinTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            patch_size=patch_size, in_chans=in_chans, embed_axis=embed_dim,
        ...
        if self.ape:
            ...
            self.absolute_pos_embed = nn.Parameter(paddle.zeros(1, embed_dim, patches_resolution[0], patches_resolution[1]))
    ...
    def forward(self, tensor_list):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                out = x_out.reshape(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                m = tensor_list.mask
...
def build_backbone(args):
    return SwinTransformer(
                 ...
                 ape=False,
                 patch_norm=True,
                 out_indices=(0, 1, 2, 3) if args.num_feature_levels > 1 else (3,),
                 frozen_stages=-1,
                 use_checkpoint=False,
