...
class ObjectDecoder(nn.Layer):
    ...
    def forward(self, srcs, mask, targets=None, is_training=True, additional_info={}, kwargs={}):
        """
        ...
        for lid, layer in enumerate(self.object_decoder_layers):
            ...
            dec_output.append(tgt_object)

        for tgt_object in dec_output:
            if self.training or self.refine_reference_points or lid == self.num_layers - 1:
                ...
                layer_outputs, layer_loss = self.detect_head[lid](tgt_object, query_pos_embed, reference_points, srcs=srcs, src_padding_masks=mask, is_training=is_training, **predictor_kwargs)
                if self.refine_reference_points:
