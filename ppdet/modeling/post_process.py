...
from .transformers.utils import box_cxcywh_to_xyxy
try:
...
__all__ = [
    ...
    'CenterNetPostProcess', 'DETRPostProcess', 'SparsePostProcess', 'PostProcess_new', 'MutiClassPostProcess', 'KeypointPostProcess'
]
...
@register
class PostProcess_new(object):
    """ This module converts the model's output into the format expected by the coco api"""
    def __init__(self):
        super(PostProcess_new, self).__init__()
...
@register
class MutiClassPostProcess(object):
    def __init__(self):
        super(MutiClassPostProcess, self).__init__()
    ...
    def __call__(self, outputs, target_sizes):
        ...
        if "detection" in outputs:
            ...
            all_boxes = box_cxcywh_to_xyxy(output["pred_boxes"]) * box_scale[:, None, :]
            results_det = []
...
@register
class KeypointPostProcess(object):
    def __init__(self):
        super(KeypointPostProcess, self).__init__()
    ...
    def __call__(self, outputs, target_sizes):
        ...
        labels = paddle.zeros_like(scores, dtype=paddle.int64)
        ...
        if "pred_boxes" in outputs:
            ...
            boxes = box_cxcywh_to_xyxy(out_bbox) # b, obj, 4
