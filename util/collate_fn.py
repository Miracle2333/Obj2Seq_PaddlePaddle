...
class CLSCollator():
    ...
    def __call__(self, batch):
        ...
        if "boxes_by_cls" in batch[1][0]:
            # process targets
            overall_batch = dict()
            overall_batch["image_id"] = paddle.stack([item["image_id"] for item in batch[1]])
            overall_batch["size"] = paddle.stack([item["size"] for item in batch[1]])
            overall_batch["orig_size"] = paddle.stack([item["orig_size"] for item in batch[1]])
            ...
            if "object_class" in batch[1][0]:
                overall_batch["object_class"] = paddle.stack([item["object_class"] for item in batch[1]])
